...
pub unsafe fn client_read<T>(sqe: &mut IoUring, socket_fd: i32, buffer: Rc<UnsafeCell<T>>)
where
    T: Readable,
{
    ...
    let read = opcode::Recv::new(
        types::Fd(socket_fd),
        buffer_ptr,
        crate::client::BUFFER_SIZE as u32,
    )
    .build()
    .user_data(user_data);
...
pub unsafe fn client_close<T>(sqe: &mut IoUring, socket_fd: i32, buffer: Rc<UnsafeCell<T>>)
where
    T: Readable,
{
...
pub fn completion_queue(cqe: &mut IoUring) -> CompletionQueueMessage {
    ...
    match msg_type {
        0 => CompletionQueueMessage::ClientConnected(syscall_ret_value as i32),
        1 => CompletionQueueMessage::MessageReceived(client_fd as i32, syscall_ret_value),
        2 => CompletionQueueMessage::MessageSent(client_fd as i32),
        3 => CompletionQueueMessage::ClientClosed(client_fd as i32),
        _ => panic!("help me"),
    }
}
